...
function Graph(method, url, postData, callback) {
  ...
  if(typeof this.options.encoding == "undefined"){
    // Assign value to the property here
    this.options.outputEncoding = 'utf-8';
  } else {
    this.options.outputEncoding = this.options.encoding;
  ...
  this.allowParseJson         = allowParseJson;
...
Graph.prototype.end = function (body, statusCode) {
  ...
  if (!json) {
    try {
      ...
      if (~body.indexOf('{') && ~body.indexOf('}')) {
        if( statusCode >= 400 || this.allowParseJson )
        {
...
Graph.prototype.get = function () {
  ...
  return request.get(uri, this.options, function(err, res, body) {
    if (err) {
      self.callback({
    ...
    self.end(body, res.statusCode | 0);
  });
};
...
Graph.prototype.post = function() {
  ...
  var uri = this.options.uri;
  delete this.options.uri;
  this.options.payload = postData;
  return fetch.fetchUrl(uri, this.options, function (err, res, body) {
    if (err) {
      self.callback({
        message: 'Error processing https request'
        , exception: err
...
exports.allowParseJson = function(value)
{
  allowParseJson = value;
}